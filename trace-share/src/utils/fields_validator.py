class FieldsValidator:
    @staticmethod
    def validate(object, required_fields=None, dependent_fields=None):
        if required_fields:
    ...
    @staticmethod
    def validate_required_fields(object, required_fields):
        ...
        for field, sub_fields in required_fields.items():
            if field not in object or object.get(field) is None:
                raise KeyError(f"Missing required field: {field}")
            FieldsValidator.validate_required_fields(object.get(field), sub_fields)


    @staticmethod
    def validate_dependent_fields(object, dependent_fields):
        ...
        for field, sub_fields in dependent_fields.items():
            if field in object and object.get(field) is not None:
                FieldsValidator.validate_dependent_fields(object.get(field), sub_fields)
